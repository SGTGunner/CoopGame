...
FVector ASTrackerBot::GetNextPathPoint()
{
	...
	for (FConstPawnIterator It = GetWorld()->GetPawnIterator(); It; ++It)
	{
		...
		USHealthComponent* TesPawnHealthComp = Cast<USHealthComponent>(TestPawn->GetComponentByClass(USHealthComponent::StaticClass()));
		if (TesPawnHealthComp && TesPawnHealthComp->GetHealth() > 0.0)
		{
...
void ASTrackerBot::SelfDestruct()
{
	...
	if (Role == ROLE_Authority)
	{
		...
		{
			DrawDebugSphere(GetWorld(), GetActorLocation(), ExplosionRadius, 12, FColor::Red, false, 2.0f, 2.0f);
		}
